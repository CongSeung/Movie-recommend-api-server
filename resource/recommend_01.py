...
class MovieRecommendResource(Resource):
    ...
    def get(self):
        ...
        # DB로부터 사져온, 내 별점 정보를 데이터프레임으로 만들어준다.
        ...
        # 중복된 영화는, Weight가 가장 큰 값으로 선택해준다.
        ...
        # 내가 이미 봐서 별점을 남긴 영화는 위 리스트에서 제외해야 한다.
        similar_movie_list.reset_index(inplace=True)

        # 내가 이미 본 영화 제목만 가져온다.
        title_list = df_my_rating['title'].tolist()

        # similar_movie_list에 내가 본 영화인 title_list를 제외하고 가져온다.
        similar_movie_list['title'].isin(title_list)

        # True False에서 맨 앞에 ~ 넣으면 뜻이 반전된다.
        recommend_movie_list = similar_movie_list.loc[ ~similar_movie_list['title'].isin(title_list) , ]

        # 추천 영화 20개 가져오기
        print(recommend_movie_list.iloc[ 0:19+1 ,  ])

        # 판다스 데이터프레임을 json으로 바꿀 때는 to_dict('records')를 쓰면 된다.
        return {'result' : 'success' ,
                'movie_list' : recommend_movie_list.iloc[ 0:19+1 ,  ].to_dict('records')}
